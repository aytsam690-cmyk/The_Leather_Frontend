...
import { Star, CheckCircle, XCircle, Clock, Search, Filter, Plus, Trash2, X, Edit2 } from 'lucide-react';
...
export default function Reviews() {
  ...
  const [createForm, setCreateForm] = useState({ product: '', name: '', rating: 5, comment: '', isFeatured: false });
  ...
  const openCreateModal = () => {
    setIsEditing(false);
    setEditId(null);
    setCreateForm({ product: '', name: '', rating: 5, comment: '', isFeatured: false });
    setImages([]);
    setImagesToRemove([]);
  ...
  const handleSave = async () => {
    if ((!isEditing && !createForm.product) || !createForm.name.trim()) return;
    setSaving(true);
    
    const formData = new FormData();
    if (!isEditing) formData.append('product', createForm.product);
    formData.append('name', createForm.name);
    formData.append('rating', createForm.rating);
    formData.append('comment', createForm.comment);
    formData.append('isFeatured', createForm.isFeatured);
    
    // Add new images (those with 'preview' are newly added files)
    const newFiles = images.filter(img => img.preview);
    newFiles.forEach(file => {
      formData.append('images', file);
    });

    // Add images to remove (only for edit)
    if (isEditing && imagesToRemove.length > 0) {
      imagesToRemove.forEach(pubId => {
        formData.append('imagesToRemove', pubId);
      });
    }

    try {
      if (isEditing) {
        await adminApi.put(`/reviews/${editId}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      } else {
        await adminApi.post('/reviews/admin', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
  ...
  const filteredReviews = reviews.filter(r => {
    ...
    if (filter === 'featured' && !r.isFeatured) return false;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <button onClick={openCreateModal} className="flex items-center gap-1.5 px-3 py-1.5 text-white text-xs font-semibold rounded-sm hover:opacity-90 transition-all" style={{ background: CORAL }}>
            ...
            {filteredReviews.map(review => (
              <div key={review._id} className="p-5 flex flex-col sm:flex-row gap-5 hover:bg-[#F8F8F6]/50 transition-colors">
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {review.isFeatured && (
                          <span className="bg-[#C9A96E] text-white text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-sm">Featured</span>
                        )}
                      </div>
                  ...
                  {review.images && review.images.length > 0 && (
                    <div className="flex gap-2 mt-3">
                      {review.images.map(img => (
                        <div key={img.publicId || img.url} className="w-12 h-12 rounded-sm border border-[#E8E8E4] overflow-hidden shrink-0">
                          <img src={img.url} className="w-full h-full object-cover" alt="review attachment" />
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="sm:w-48 shrink-0 flex flex-col sm:items-end justify-center gap-2 border-t sm:border-t-0 sm:border-l border-[#E8E8E4] pt-4 sm:pt-0 sm:pl-5">
                  ...
                  <div className="flex gap-2 w-full sm:justify-end flex-wrap sm:flex-nowrap">
                    {!review.isApproved && (
                      <button 
                    ...
                    <button 
                      ...
                      className="px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-sm text-xs font-semibold transition-colors flex-1 sm:flex-none text-center"
                    >
                  ...
                  <button 
                    ...
                    className="px-3 py-1.5 bg-red-50 text-red-500 hover:bg-red-100 rounded-sm text-xs font-semibold transition-colors w-full sm:w-auto text-center mt-1"
                  >
                    Delete
                  </button>
                </div>
        ...
        {showCreateModal && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowCreateModal(false)}>
            <motion.div
              ...
              className="bg-white rounded-sm border border-[#E8E8E4] p-6 w-full max-w-md max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-[#111111]">{isEditing ? 'Edit Review' : 'Add Review'}</h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#6B6B6B] mb-1.5">Review Images</label>
                  <ImageUploader 
                    images={images} 
                    setImages={(newImages) => {
                      // Check if any existing image was removed
                      if (isEditing) {
                        const newUrls = newImages.map(i => i.publicId).filter(Boolean);
                        const removed = (images || []).filter(i => i.publicId && !newUrls.includes(i.publicId));
                        if (removed.length > 0) {
                          setImagesToRemove(prev => [...prev, ...removed.map(i => i.publicId)]);
                        }
                      }
                      setImages(newImages);
                    }} 
                ...
                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-[#E8E8E4]">
                  <input 
                    type="checkbox" 
                    id="isFeatured" 
                    checked={createForm.isFeatured}
                    onChange={e => setCreateForm(f => ({ ...f, isFeatured: e.target.checked }))}
                    className="w-4 h-4 rounded-sm border-[#D0D0CA] text-[#C9A96E] focus:ring-[#C9A96E]"
                  />
                  <label htmlFor="isFeatured" className="text-sm font-semibold text-[#111111] cursor-pointer">
                    Feature on Homepage Testimonials
                  </label>
                </div>
              </div>
              <div className="flex gap-3 mt-5">
                ...
                <button onClick={handleSave} disabled={saving || (!isEditing && !createForm.product) || !createForm.name.trim()} className="flex-1 px-4 py-2.5 rounded-sm text-sm font-semibold text-white hover:opacity-90 transition-all disabled:opacity-50" style={{ background: CORAL }}>{saving ? 'Saving...' : 'Save Review'}</button>
              </div>
            </motion.div>
          </div>
        )}
