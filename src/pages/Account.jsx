  ...
  return (
    <div className="bg-white border border-[#E8E8E4] rounded-sm p-8">
      ...
      <form onSubmit={handleSave}>
        ...
        <button type="submit" disabled={saving} className="bg-[#111111] hover:bg-[#333333] text-white border border-[#111111] rounded-sm px-8 py-3 font-dm font-medium text-sm uppercase tracking-[0.04em] transition-colors mt-6">
      ...
      <div className="flex gap-0 border-b border-[#E8E8E4] mb-6">
        {['All', 'Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'].map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`font-dm text-[11px] uppercase tracking-[0.08em] px-5 py-3 cursor-pointer border-b-2 -mb-px transition-colors ${filter === f ? 'text-[#111111] border-[#111111]' : 'text-[#9E9E9E] border-transparent hover:text-[#6B6B6B]'}`}
          >
            {f}
          </button>
        ))}
      ...
      <div className="space-y-4">
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white border border-[#E8E8E4] rounded-sm p-6 hover:border-[#D0D0CA] transition-all duration-250">
                <div className="flex justify-between items-start cursor-pointer" onClick={() => setExpanded(isOpen ? null : order.id)}>
                  <div>
                ...
                <div className="flex gap-2 mt-4">
                  {order.items.slice(0, 4).map((item, i) => (
                    <div key={i} className="w-14 h-14 border border-[#E8E8E4] rounded-sm object-cover bg-[#F8F8F6] overflow-hidden">
                  ...
                  {order.items.length > 4 && (
                    <div className="w-14 h-14 border border-[#E8E8E4] rounded-sm bg-[#F8F8F6] flex items-center justify-center font-dm text-[11px] text-[#9E9E9E]">
                      +{order.items.length - 4} more
                    </div>
                  )}
                </div>

                <div className="mt-4 pt-4 border-t border-[#E8E8E4] flex justify-between items-center">
                  <p className="font-cormorant text-[18px] font-medium text-[#111111]">${order.total.toFixed(2)}</p>
                  <div className="flex gap-3">
                    {order.status === 'pending' && (
                      <button className="bg-transparent text-[#9B2226] hover:text-[#7B1215] text-[12px] uppercase tracking-[0.06em] font-medium border-none cursor-pointer px-4">Cancel</button>
                    )}
                    <button onClick={() => setExpanded(isOpen ? null : order.id)} className="bg-transparent text-[#6B6B6B] hover:text-[#111111] text-[12px] uppercase tracking-[0.06em] font-medium border-none cursor-pointer px-4">
      ...
      {addresses.map((addr) => (
        <div key={addr.id} className="bg-white border border-[#E8E8E4] rounded-sm p-5 relative hover:border-[#D0D0CA] transition-all duration-250">
          ...
          <div className="flex gap-4 mt-4 pt-4 border-t border-[#E8E8E4]">
            <button className="bg-transparent border-none font-dm text-[11px] uppercase tracking-[0.06em] text-[#9E9E9E] hover:text-[#111111] cursor-pointer p-0">Edit</button>
            <button className="bg-transparent border-none font-dm text-[11px] uppercase tracking-[0.06em] text-[#9E9E9E] hover:text-[#111111] cursor-pointer p-0" onClick={() => setAddresses(a => a.filter(x => x.id !== addr.id))}>Delete</button>
          </div>
        </div>
      ))}
  ...
  return (
    <div className="bg-white border border-[#E8E8E4] rounded-sm p-8">
      ...
      <form className="max-w-md space-y-5">
        ...
        <button type="button" className="bg-[#111111] hover:bg-[#333333] text-white border border-[#111111] rounded-sm px-8 py-3 font-dm font-medium text-sm uppercase tracking-[0.04em] transition-colors mt-6">
          Update Password
        </button>
      </form>
    </div>
    ...
    <div className="min-h-screen bg-[#F8F8F6]">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-8">
          <div className="bg-white border border-[#E8E8E4] rounded-sm p-6 self-start sticky top-24">
            <div className="text-center pb-6 border-b border-[#E8E8E4] mb-6">
            ...
            <nav className="space-y-0.5">
              {NAV_TABS.map(({ id, label, icon: Icon }) => (
                <button key={id} onClick={() => setActiveTab(id)} className={`w-full flex items-center gap-3 px-4 py-3 font-dm text-[13px] cursor-pointer rounded-sm transition-colors border-none ${activeTab === id ? 'bg-[#F8F8F6] text-[#111111] font-medium' : 'bg-transparent text-[#6B6B6B] hover:bg-[#F8F8F6] hover:text-[#111111]'}`}>
                  <Icon size={16} />
                  {label}
                </button>
              ))}
            ...
            <button onClick={handleLogout} className="mt-6 pt-6 border-t border-[#E8E8E4] border-l-0 border-r-0 border-b-0 bg-transparent flex items-center gap-3 font-dm text-[13px] text-[#9B2226] hover:text-[#7B1215] cursor-pointer px-4 py-2.5 w-full rounded-sm hover:bg-[#FFF1F2] transition-colors">
